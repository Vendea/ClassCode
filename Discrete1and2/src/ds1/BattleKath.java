...
public class BattleKath implements ActionListener{
	...
	boolean playOnNetwork = true; // XXX network
	ds1.GameClient gameClient; // XXX network
	int myPlayerNumber; // XXX network
	Semaphore holdForHuman; // XXX network
	Semaphore holdForGUI; // XXX network
	String myRecentMove; // XXX network
	String myNetworkName; // XXX network
	String otherPlayerName; // XXX network
	...
	public BattleKath() {
		...
		for(int i = 0; i < 10; i++){
			for(int j = 0; j < 10; j++){
				...
				attackrowLabelOne[i] = new JLabel(""+(char)(i+65),
						JLabel.CENTER);
				attackrowLabelTwo[i] = new JLabel(""+(char)(i+65),
						JLabel.CENTER);
				...
				if(j<9){
					...
					columnLabelTwo[j] = new JLabel(""+(char)(j+49),
							JLabel.CENTER);
					attackcolumnLabelOne[j] = new
							JLabel(""+(char)(j+49), JLabel.CENTER);
					attackcolumnLabelTwo[j] = new
							JLabel(""+(char)(j+49), JLabel.CENTER);
				}
				if(j == 9){
		...
		holdForHuman = new Semaphore(0); // XXX network
		holdForGUI = new Semaphore(0); // XXX network
		myNetworkName = "Kat"; // XXX Network. Default name in honor of Katherine Beine, the author of Battleship
		otherPlayerName = "The enemy"; // XXX network default name
	...
	public void newGame(){
		...
		// XXX Network Play
		if(playOnNetwork){
			gameClient = new ds1.GameClient(12);
			gameClient.connectToServer();
			myNetworkName = JOptionPane.showInputDialog("Please enter your name.");
			myPlayerNumber = gameClient.startGame(myNetworkName);
			otherPlayerName = gameClient.getOtherPlayerName();
		}
		...
		JOptionPane.showMessageDialog(null,"Click a square to start placing your ships "
				...
				"Begin battleship - place ships",
				JOptionPane.INFORMATION_MESSAGE);

		if(playOnNetwork){ // XXX Start the Network game
			turn = myPlayerNumber;
			networkPlay();
		}
	}
	 ...
	 */
	public Boolean checkShipPlace(int t, int shipid, int temprow, int
			tempcol, int dir){
		int place[][];
		Boolean rv = true;
		Boolean up = true;
	 ...
	 */
	public void finishShipPlacement(int t, int shipid, int srow, int
			scol, int trow, int tcol) {
		JButton[][] buttons;
		int place[][];
		Ship thing = whichShip(shipid);
		...
		if(cplay){
			...
			if(playOnNetwork){
				if(myPlayerNumber == 1){
					...
					attackBoardGUIOne.setVisible(true);
					myBoardGUIOne.setVisible(true);
					statusTwo.setVisible(false);
					attackBoardGUITwo.setVisible(false);
					myBoardGUITwo.setVisible(false);
					gameClient.sendMove(shipLocationString(2));
					String otherGuysMoves = gameClient.getMove();
					stringPlaceShips(otherGuysMoves, 2);
					for(int i = 0; i < 10; i++)
						for(int j = 0; j < 10; j++){
							attackShipButtonOne[i][j].setEnabled(true);

							//attackShipButtonOne[i][j].setBackground(Color.white);
						}
				} else {
					...
					myBoardGUITwo.setVisible(true);
					statusOne.setVisible(false);
					attackBoardGUIOne.setVisible(false);
					myBoardGUIOne.setVisible(false);
					gameClient.sendMove(shipLocationString(1));
					String otherGuysMoves = gameClient.getMove();
					stringPlaceShips(otherGuysMoves, 1);
					for(int i = 0; i < 10; i++)
						for(int j = 0; j < 10; j++){
	...
	private void networkPlay(){
		...
		while(true){
			if(turn == myPlayerNumber){
				...
				gameClient.sendMove(myRecentMove);
				play(myPlayerNumber, (int)(myRecentMove.charAt(0) - '0'),
						(int)(myRecentMove.charAt(1) - '0'));
			}
			else {
				String otherMove = gameClient.getMove();
				play(3-myPlayerNumber, (int)(otherMove.charAt(0) - '0'),
						(int)(otherMove.charAt(1) - '0'));
			}
			int e = endCheck();
			if((e == TwoPlayer.PLAYER1WIN && myPlayerNumber == 1) ||
					(e == TwoPlayer.PLAYER2WIN && myPlayerNumber == 2)){
				System.out.println("We win!");
				gameClient.declareWin();
				break;
			}
			if(e == TwoPlayer.DRAW){
	...
	public void stringPlaceShips(String s, int t){
		String small = s;
		int srow = 0;
		int scol = 0;
		int trow = 0;
		...
		for(int j = 0; j <5; j++){
			srow = Integer.parseInt(small.charAt(0)+"");
			scol = Integer.parseInt(small.charAt(1)+"");
			trow = Integer.parseInt(small.charAt(2)+"");
			tcol = Integer.parseInt(small.charAt(3)+"");
			small = small.substring(4,small.length());

			if(srow == trow){
				if(tcol<scol)//go left
					for(int i = 0; i < scol-tcol+1; i++){

						place[srow][scol-i] = SHIP;
					}
				if(scol<tcol)//go right
					for(int i = 0; i < tcol-scol+1; i++){
			...
			if(scol == tcol){
				if(trow<srow)//go up
					for(int i = 0; i < srow-trow+1; i++){
						place[srow-i][scol] = SHIP;
					}
				if(srow<trow)//go down
					for(int i = 0; i < trow-srow+1; i++){
						place[srow+i][scol] = SHIP;
					}
			}
			whichShip(j).setLocation(t,srow, scol, trow, tcol);
			shipsPlaced++;
		}
	}

	private String shipLocationString(int t){
		String rv="";
		for(int i = 0; i < 5; i++){
			Ship thing = whichShip(i);
			rv+=thing.returnRow(t, 0);
			rv+=thing.returnCol(t, 0);
			rv+=thing.returnRow(t, thing.getLength()-1);
			rv+=thing.returnCol(t, thing.getLength()-1);
		}
		return rv;
	}
				...
				for(int i=0;i<10;i++){
					for(int j=0; j<10; j++){
						...
						if(attackShipButtonOne[i][j] == e.getSource()){
							...
							if(numMoves == 17){
								int sw = endCheck();
								if(sw!=TwoPlayer.PLAYER1WIN && sw!=
										TwoPlayer.PLAYER2WIN){
			...
			if(!placingShips){
				...
				find:
					for(int i=0;i<10;i++)
						for(int j=0;j<10;j++)
							if(button[i][j] == e.getSource()){
								finishShipPlacement(turn, shipsPlaced,
			...
			find:
				for(int i = 0; i < 5; i++){
					...
					for(int j = 0; j < s.getLength(); j++){
						if(s.returnRow(t,j) == srow &&
								s.returnCol(t,j) == scol){
							thing = s;
							break find;
						}
					}
			...
			do{
				...
				if(ssss.contains(num)){
					...
					if(checkShipPlace(2, i, srow, scol, lrud)){
						thir = true;
						if(lrud == 0)
							finishShipPlacement(2, i, srow, scol,
									srow, scol-(whichShip(i).getLength()-1));
						if(lrud == 1)
							finishShipPlacement(2, i, srow, scol,
									srow, scol+(whichShip(i).getLength()-1));
						if(lrud == 2)
							finishShipPlacement(2, i, srow, scol,
									srow-(whichShip(i).getLength()-1), scol);
						if(lrud == 3)
							finishShipPlacement(2, i, srow, scol,
									srow+(whichShip(i).getLength()-1), scol);
					}
