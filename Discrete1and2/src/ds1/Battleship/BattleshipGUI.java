...
public class BattleshipGUI extends TwoPlayer implements ActionListener {
	...
	int fourShip;
	int fiveShip;
	int[][] shipArray;		//array containing positions and status of human ships
	...
	public Boolean validPlace(int row, int col){
		...
		if(prevRow==row && prevRow!=-1 && row>0 && row<9){
			shipButton[row-1][col].setEnabled(false);
			shipButton[row+1][col].setEnabled(false);
		}
		if(prevCol==col && prevCol!=-1 && col>0 && col<9){
	...
	public void checkMyShips(){
		String humanShipString = "HUMAN SHIPS \n";
		String CPUShipString = "CPU SHIPS \n";
		
		for(int i=0;i<10;i++){
			for(int j=0;j<10;j++){
				humanShipString += ""+shipArray[i][j];
				if(j==9)
					humanShipString += "\n";
			}
		}
		System.out.println(humanShipString);
		
		for(int i=0;i<10;i++){
			for(int j=0;j<10;j++){
				CPUShipString += ""+CPUShipArray[i][j];
				if(j==9)
					CPUShipString += "\n";
			}
		}
		System.out.println(CPUShipString);
		
		
		
	}

	public void humanMove(int turn){ ///needs to be modified to fit origionalhuman move
		// use row and column global variables instead of i and j
		if(CPUShipArray[row][column]==2){
			CPUShipArray[row][column]=3;
			CPUshipButton[row][column].setBackground(Color.red);
			CPUshipButton[row][column].removeActionListener(this);

		}
		if(CPUShipArray[row][column]==1){
			CPUShipArray[row][column]=4;
			CPUshipButton[row][column].setBackground(Color.yellow);
			CPUshipButton[row][column].removeActionListener(this);
		}

		System.out.println("HUMAN TURN DONE"+row+" "+column+"   turn = "+turn);
		turn++;
	}
	...
	void computerMove(int turn) {
		...
		if(shipArray[rndRow][rndCol]==2){
			shipArray[rndRow][rndCol] = 3;
			shipButton[rndRow][rndCol].setBackground(Color.red);
		}
		if(shipArray[rndRow][rndCol]==1){
			shipArray[rndRow][rndCol] = 4;
			shipButton[rndRow][rndCol].setBackground(Color.yellow);
		}

		System.out.println("CPU TURN DONE"+rndRow+" "+rndCol);
		turn++;
	...
	public void end(int winner){
		if(winner==1)
			  JOptionPane.showMessageDialog(null,"Winner", "You Win!", JOptionPane.INFORMATION_MESSAGE);
		if(winner==2)
			JOptionPane.showMessageDialog(null, "Loser", "You Lose", JOptionPane.INFORMATION_MESSAGE);
	...
	@Override
	String boardHash(Object b) {             ////cant eval board.....
		// TODO Auto-generated method stub
		return null;
	}

	@Override
	int evaluateBoard(Object b) {          ////cant eval board.....
		// TODO Auto-generated method stub
		return 0;
	}

	@Override
	int whoseTurn(Object b) {			/////handeled in clicked method. human turn is always followed by CPU turn
		// TODO Auto-generated method stub
		return 0;
	}
