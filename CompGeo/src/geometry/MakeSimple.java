...
public class MakeSimple {
	...
	public static ArrayList<int[]> convexulate(int[] d1, ArrayList<int[]> chords, int np){
		int[] d2 = new int[np];
		for(int i = 0; i < np; i++){
			d2[i] = d1[i];
		}
		...
		for(int i = 0; i < chords.size() ;i++){
			...
			int second = ti[0]<ti[1] ? ti[1] : ti[0];
			int stop = np;

			for(int j = 0; j <= first; j++){
				dl.add(d2[j%np]);
			}
			for(int j = second; j< stop;j++){
				dl.add(d2[j%np]);
			}
			...
			int[] dleft = new int[dl.size()];
			int[] dright = new int[dr.size()];
	...
	public static ArrayList<int[]> getChords(int[] D2, int nP){
	...
	static boolean isConvex(int[] d1, int np){
		int[] d2 = makeCounterclockwise(d1, np);
		for(int i= 1; i <= np;i++){
	...
	public static boolean chordInside(int[] P, int s, int e, int n){
		...
		s = s%n;
		e = e%n;
		if(sigma(x[P[before]], y[P[before]], x[s], y[s], x[P[after]], y[P[after]]) > 0){
			if(sigma(x[P[before]], y[P[before]], x[s], y[s], x[e], y[e]) > 0 &&
					sigma(x[e], y[e], x[s], y[s], x[P[after]], y[P[after]]) > 0)
				return true;
			else 
				return false;
		} else {
			if(sigma(x[P[before]%n], y[P[before]%n], x[s], y[s], x[e], y[e]) > 0 ||
					sigma(x[e], y[e], x[s], y[s], x[P[after]%n], y[P[after]%n]) > 0)
				return true;
			else 
				return false;
		}
	...
	public static boolean chordCrossesBoundary(int[] P, int s, int e, int n){
		for(int i = 0; i < n; i++){
			if(crosses(x[P[i]], y[P[i]], x[P[(i+1)%n]], y[P[(i+1)%n]], x[s], y[s], x[e], y[e]))
				return true;
		}
		return false;
	}
