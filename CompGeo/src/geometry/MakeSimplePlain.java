...
public class MakeSimplePlain {
	...
	static ArrayList<int[]> getChords(int[] d2, int nP){
		...
		for(int i = 0; i < nP - 2; i++){
			for(int j = i+2; j < nP - (i==0 ? 1 : 0); j++){
				if(chordInside(i, j, nP) && !chordCrossesBoundary(d2, i, j, nP)){
					int[] chord = {d2[i], d2[j]};
					rval.add(chord);
				}
			}
		...
		for(int i = rval.size()-1;i>=0;i--){
			...
			double tmx = x[rval.get(i)[0]];
			double tmy = y[rval.get(i)[0]];
			double bmx = x[rval.get(i)[1]];
			double bmy = y[rval.get(i)[1]];
			if(sigma(x[topbefore], y[topbefore], tmx, tmy, x[topafter], y[topafter])!=-1){
				if(sigma(x[bbefore], y[bbefore], bmx, bmy, x[bafter], y[bafter])!=-1){
					rval.remove(i);
				}
			}
		}
	 ...
	 */
	public static boolean chordInside(int s, int e, int n){
		int before = (s + n - 1) % n;
		int after  = (s + 1) % n;
	...
	public static class ConvexulationPanel extends JPanel{
		public void paintComponent(Graphics g) {
			...
			// Now draw the points
			for (int i = 0; i < numPoints; i++) {
				g2.fillRect((int) (Xwid * x[D[i]]) - POINTWID, size.height - (int) (Ywid * y[D[i]])
						- POINTWID, 2 * POINTWID + 1, 2 * POINTWID + 1);
			}
