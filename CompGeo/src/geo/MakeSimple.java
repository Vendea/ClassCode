...
public class MakeSimple {
    ...
    static ArrayList<Integer> D = new ArrayList<Integer>(); // the one that becomes very simple
    ...
    public static void createVerySimplePolygon() {
        ...
        while (stillTrying) {
            ...
            while (hasCrossing) {
                ...
                for (int i = 0; (i < numPoints - 2); i++)
                    for (int j = i + 2; j < numPoints; j++)
                        if (crosses(x[D.get(i)], y[D.get(i)], x[D.get(i+1)], y[D.get(i+1)],
                                x[D.get(j)], y[D.get(j)], x[D.get((j + 1) % numPoints)],
                                y[D.get((j + 1) % numPoints)])) {
                            hasCrossing = true;
                            int k = i + 1;
                            int l = j;
                            while (l > k) {
            ...
            for (int i = 0; i < numPoints; i++)
                for (int j = (i + 1) % numPoints; ((j + 2) % numPoints) != i; j = ((j + 1) % numPoints))
                    if (dis2(D.get((i + numPoints - 1) % numPoints), D.get(i), D.get((i + 1) % numPoints)) +
                            dis(D.get(j), D.get((j + 1) % numPoints)) >
                    dis(D.get((i + numPoints - 1) % numPoints), D.get((i + 1) % numPoints))
                    + dis2(D.get(j), D.get(i), D.get((j + 1) % numPoints))) {
                        stillTrying = true;
                        if (i < j) {
                            int temp = D.get(i);
                            for (int k = i; k < j; k++)
                                D.add(k, (k + 1) % numPoints);
                            D.add(j, temp);
                        } else {
                            int temp = D.get(i);
                            for (int k = i; k > j + 1; k--)
                                D.add(k, D.get((k + numPoints - 1) % numPoints));
                            D.add((j + 1), temp);
                        }
                    }
        }
    ...
    static ArrayList<int[]> convexulate(ArrayList<Integer> d1, ArrayList<int[]> chords, int np){
    	int[] d2 = new int[d1.size()];
    	...
    	for(int i = 0; i < chords.size() ;i++){
    		...
    		ArrayList<int[]> cleft = getChords(dl, x, y, dleft.length);
    		ArrayList<int[]> cright = getChords(dr, x, y, dright.length);
    		
    		ArrayList<int[]> templeft = convexulate(dl, cleft, dleft.length );
    		ArrayList<int[]> tempright =convexulate(dr, cright, dright.length);
    ...
    static ArrayList<int[]> getChords(ArrayList<Integer> d1, double[]tx, double[] ty, int nP){
    	int[] d2 = new int[d1.size()];
     ...
     */
    public static void makeCounterclockwise(ArrayList<Integer> P, int n){
    ...
    public static class VerySimplePanel extends JPanel {
        public void paintComponent(Graphics g) {
            ...
            for (int i = 0; i < numPoints; i++) {
                ...
                if (i > 1)
                    g2.setColor(Color.red);
                g2.drawLine((int) (Xwid * x[D.get(i)]), size.height - (int) (Ywid * y[D.get(i)]),
                        (int) (Xwid * x[D.get((i + 1) % numPoints)]),
                        size.height - (int) (Ywid * y[D.get((i + 1) % numPoints)]));
            }
            // Now draw the points
            for (int i = 0; i < numPoints; i++) {
                g2.fillRect((int) (Xwid * x[i]) - POINTWID, size.height - (int) (Ywid * y[i])
    ...
    public static class ConvexulationPanel extends JPanel{
        public void paintComponent(Graphics g) {
            ...
            for (int i = 0; i < numHullPoints; i++) {
                g2.drawLine((int) (Xwid * x[D.get(i)]), size.height - (int) (Ywid * y[D.get(i)]),
                        (int) (Xwid * x[D.get((i + 1) % numHullPoints)]),
                        size.height - (int) (Ywid * y[D.get((i + 1) % numHullPoints)]));
            }
            // Now draw the points
            for (int i = 0; i < numPoints; i++) {
                g2.fillRect((int) (Xwid * x[i]) - POINTWID, size.height - (int) (Ywid * y[i])
            ...
            for(int i = 0; i < numConvexulationChords; i++){
                ...
                g2.drawLine((int) (Xwid * x[D.get(chord[0])]), size.height - (int) (Ywid * y[D.get(chord[0])]),
                        (int) (Xwid * x[D.get(chord[1])]),
                        size.height - (int) (Ywid * y[D.get(chord[1])]));
            }
        }
    }
}